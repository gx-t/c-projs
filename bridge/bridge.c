...
static int b_show_usage(const char* txt, int code)
{
  if(txt) fputs(txt, stderr);
  const char* msg = "Usage: bridge <mode> ...\n"
  "Mode can be one of:\n"
  "  help               - display this text\n"
  "  single             - no child processes\n"
  "  pool               - pool of fixed number of child processes\n"
  "  dynamic            - do fork for each connection\n";
  fputs(msg, stderr);
  return code;
}

static int b_single_usage(const char* txt, int code)
{
  if(txt) fputs(txt, stderr);
  const char* msg = "Usage: bridge single help,\n"
  "       bridge single <port> <executable> ...\n"
  "  help               - display this text\n"
  "  port               - listen port 1-65535\n"
  "  executable         - absolute path to executable (bash, etc.)\n"
  "  ...                - args to be passed to executable\n";
  fputs(msg, stderr);
  return code;
}
...
static int b_init_listen(int port)
{
  ...
  if(bind(ss, (struct sockaddr *) &sin, sizeof(sin)) < 0 || listen(ss, 4) < 0)
  {
    close(ss);
    perror("bind/listen");
    return -1;
...
static int b_single(int argc, char** argv)
{
  if(argc < 2) return b_single_usage("Not enough arguments for single mode.\n", ERR_ARGC);
  argc--;
  argv++;
  if(!strcmp(*argv, "help")) return b_single_usage(0, ERR_OK);
  if(argc < 2) return b_single_usage("Invalid number of arguments.\n", ERR_ARGC);
  int port = atoi(*argv++);
  if(port < MIN_PORT || port > MAX_PORT) return b_single_usage("Invalid port number.\n", ERR_PORT);
  int ss = b_init_listen(port);
...
static int b_pool(int argc, char** argv)
{
  return ERR_OK;
}

static int b_dynamic(int argc, char** argv)
{
  return ERR_OK;
}
...
{
  if(argc < 2) return b_show_usage("Not enough arguments.\n", ERR_ARGC);
  argc--;
  argv++;
  if(!strcmp(*argv, "help")) return b_show_usage(0, ERR_OK);
