...
static void md5_process( struct md5_context *ctx, BYTE data[64] )
{
  DWORD A, B, C, D, X[16];
...
void md5_finish( struct md5_context *ctx, DWORD digest[4] )
{
  ...
  PUT_DWORD( ctx->total[0], msglen, 0 );
  PUT_DWORD( ctx->total[1], msglen, 4 );
  ...
  digest[0] = ctx->state[0];
  digest[1] = ctx->state[1];
  digest[2] = ctx->state[2];
  digest[3] = ctx->state[3];
}
