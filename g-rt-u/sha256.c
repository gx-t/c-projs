...
static int file_sha256(unsigned char* digest, const char* file, int (*callback)(void*, long long), void* ctx)
{
  struct sha256_ctx sh_ctx;
  ...
  if(fd == -1)
    return -1;
  sha256_starts(&sh_ctx);
  while(0 < (len = read(fd, buff, sizeof(buff))))
  {
    if(!callback(ctx, count))
    {
      close(fd);
      return 1;
    }
    sha256_update(&sh_ctx, buff, len);
    count += len;
  }
  sha256_finish(&sh_ctx, digest);
  close(fd);
  return 0;
}

struct SHA256 g_sha256 =
{
