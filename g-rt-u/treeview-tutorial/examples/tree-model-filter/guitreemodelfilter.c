...
/*************************************************************************
 *
 *  Note: this is gtktreemodelfilter.c from gtk+ CVS as of Jan 19, 2004.
 *        There might be bugfixes or updates in CVS by now.
 *
 *  The only difference between this file and the original file is
 *        that the namespace has been changed from 'gtk' to 'gui',
 *        so that it is easy to use the tree model filter with
 *        gtk+ versions < 2.4 as well (without conflicts once 2.4
 *        comes out), and the object private data stuff that needed
 *        working around, as it is only available in glib >= v2.3
 *
 *  -Tim Muller
 *
 *************************************************************************/
...
/*
#define GUI_TREE_MODEL_FILTER_GET_PRIVATE(obj)  (G_TYPE_INSTANCE_GET_PRIVATE ((obj), GUI_TYPE_TREE_MODEL_FILTER, GuiTreeModelFilterPrivate))
*/

#define GUI_TREE_MODEL_FILTER_GET_PRIVATE(obj) (obj->priv)

struct _GuiTreeModelFilterPrivate
{
  gpointer root;
  gint stamp;
  guint child_flags;
  ...
  GuiTreeModelFilterVisibleFunc visible_func;
  gpointer visible_data;
  GtkDestroyNotify visible_destroy;

  gint modify_n_columns;
  GType *modify_types;
  GuiTreeModelFilterModifyFunc modify_func;
...
#define GUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS(filter) \
        (((GuiTreeModelFilter *)filter)->priv->child_flags & GTK_TREE_MODEL_ITERS_PERSIST)
...
static void         gui_tree_model_filter_init                            (GuiTreeModelFilter      *filter);
static void         gui_tree_model_filter_class_init                      (GuiTreeModelFilterClass *filter_class);
static void         gui_tree_model_filter_tree_model_init                 (GtkTreeModelIface       *iface);
static void         gui_tree_model_filter_drag_source_init                (GtkTreeDragSourceIface  *iface);
static void         gui_tree_model_filter_finalize                        (GObject                 *object);
static void         gui_tree_model_filter_set_property                    (GObject                 *object,
                                                                           guint                    prop_id,
                                                                           const GValue            *value,
                                                                           GParamSpec              *pspec);
static void         gui_tree_model_filter_get_property                    (GObject                 *object,
                                                                           guint                    prop_id,
                                                                           GValue                 *value,
                                                                           GParamSpec             *pspec);

/* signal handlers */
static void         gui_tree_model_filter_row_changed                     (GtkTreeModel           *c_model,
                                                                           GtkTreePath            *c_path,
                                                                           GtkTreeIter            *c_iter,
                                                                           gpointer                data);
static void         gui_tree_model_filter_row_inserted                    (GtkTreeModel           *c_model,
                                                                           GtkTreePath            *c_path,
                                                                           GtkTreeIter            *c_iter,
                                                                           gpointer                data);
static void         gui_tree_model_filter_row_has_child_toggled           (GtkTreeModel           *c_model,
                                                                           GtkTreePath            *c_path,
                                                                           GtkTreeIter            *c_iter,
                                                                           gpointer                data);
static void         gui_tree_model_filter_row_deleted                     (GtkTreeModel           *c_model,
                                                                           GtkTreePath            *c_path,
                                                                           gpointer                data);
static void         gui_tree_model_filter_rows_reordered                  (GtkTreeModel           *c_model,
                                                                           GtkTreePath            *c_path,
                                                                           GtkTreeIter            *c_iter,
                                                                           gint                   *new_order,
                                                                           gpointer                data);
...
static guint        gui_tree_model_filter_get_flags                       (GtkTreeModel           *model);
static gint         gui_tree_model_filter_get_n_columns                   (GtkTreeModel           *model);
static GType        gui_tree_model_filter_get_column_type                 (GtkTreeModel           *model,
                                                                           gint                    index);
static gboolean     gui_tree_model_filter_get_iter                        (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           GtkTreePath            *path);
static GtkTreePath *gui_tree_model_filter_get_path                        (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);
static void         gui_tree_model_filter_get_value                       (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           gint                    column,
                                                                           GValue                 *value);
static gboolean     gui_tree_model_filter_iter_next                       (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);
static gboolean     gui_tree_model_filter_iter_children                   (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           GtkTreeIter            *parent);
static gboolean     gui_tree_model_filter_iter_has_child                  (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);
static gint         gui_tree_model_filter_iter_n_children                 (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);
static gboolean     gui_tree_model_filter_iter_nth_child                  (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           GtkTreeIter            *parent,
                                                                           gint                    n);
static gboolean     gui_tree_model_filter_iter_parent                     (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           GtkTreeIter            *child);
static void         gui_tree_model_filter_ref_node                        (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);
static void         gui_tree_model_filter_unref_node                      (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter);

/* TreeDragSource interface */
static gboolean    gui_tree_model_filter_row_draggable                    (GtkTreeDragSource      *drag_source,
                                                                           GtkTreePath            *path);
static gboolean    gui_tree_model_filter_drag_data_get                    (GtkTreeDragSource      *drag_source,
                                                                           GtkTreePath            *path,
                                                                           GtkSelectionData       *selection_data);
static gboolean    gui_tree_model_filter_drag_data_delete                 (GtkTreeDragSource      *drag_source,
                                                                           GtkTreePath            *path);

/* private functions */
static void        gui_tree_model_filter_build_level                      (GuiTreeModelFilter     *filter,
                                                                           FilterLevel            *parent_level,
                                                                           FilterElt              *parent_elt);
static void        gui_tree_model_filter_free_level                       (GuiTreeModelFilter     *filter,
                                                                           FilterLevel            *filter_level);

static GtkTreePath *gui_tree_model_filter_elt_get_path                    (FilterLevel            *level,
                                                                           FilterElt              *elt,
                                                                           GtkTreePath            *root);

static GtkTreePath *gui_tree_model_filter_add_root                        (GtkTreePath            *src,
                                                                           GtkTreePath            *root);
static GtkTreePath *gui_tree_model_filter_remove_root                     (GtkTreePath            *src,
                                                                           GtkTreePath            *root);

static void         gui_tree_model_filter_increment_stamp                 (GuiTreeModelFilter     *filter);

static gboolean     gui_tree_model_filter_visible                         (GuiTreeModelFilter     *filter,
                                                                           GtkTreeIter            *child_iter);
static void         gui_tree_model_filter_clear_cache_helper              (GuiTreeModelFilter     *filter,
                                                                           FilterLevel            *level);

static void         gui_tree_model_filter_real_unref_node                 (GtkTreeModel           *model,
                                                                           GtkTreeIter            *iter,
                                                                           gboolean                propagate_unref);

static void         gui_tree_model_filter_set_model                       (GuiTreeModelFilter     *filter,
                                                                           GtkTreeModel           *child_model);
static void         gui_tree_model_filter_set_root                        (GuiTreeModelFilter     *filter,
                                                                           GtkTreePath            *root);

static GtkTreePath *gtk_real_tree_model_filter_convert_child_path_to_path (GuiTreeModelFilter     *filter,
                                                                           GtkTreePath            *child_path,
                                                                           gboolean                build_levels,
                                                                           gboolean                fetch_childs);

static FilterElt   *gui_tree_model_filter_fetch_child                     (GuiTreeModelFilter     *filter,
                                                                           FilterLevel            *level,
                                                                           gint                    offset,
                                                                           gint                   *index);
static void         gui_tree_model_filter_remove_node                     (GuiTreeModelFilter     *filter,
                                                                           GtkTreeIter            *iter,
                                                                           gboolean                emit_signal);
static void         gui_tree_model_filter_update_childs                   (GuiTreeModelFilter     *filter,
                                                                           FilterLevel            *level,
                                                                           FilterElt              *elt);
...
GType
gui_tree_model_filter_get_type (void)
{
  ...
  if (!tree_model_filter_type)
    {
      static const GTypeInfo tree_model_filter_info =
        {
          sizeof (GuiTreeModelFilterClass),
          NULL, /* base_init */
          NULL, /* base_finalize */
          (GClassInitFunc) gui_tree_model_filter_class_init,
          NULL, /* class_finalize */
          NULL, /* class_data */
          sizeof (GuiTreeModelFilter),
          0, /* n_preallocs */
          (GInstanceInitFunc) gui_tree_model_filter_init
        };

      static const GInterfaceInfo tree_model_info =
        {
          (GInterfaceInitFunc) gui_tree_model_filter_tree_model_init,
          NULL,
          NULL
        };
...
static void
gui_tree_model_filter_class_init (GuiTreeModelFilterClass *filter_class)
{
  ...
  object_class->set_property = gui_tree_model_filter_set_property;
  object_class->get_property = gui_tree_model_filter_get_property;
...
static void
gui_tree_model_filter_tree_model_init (GtkTreeModelIface *iface)
{
  iface->get_flags = gui_tree_model_filter_get_flags;
  iface->get_n_columns = gui_tree_model_filter_get_n_columns;
  iface->get_column_type = gui_tree_model_filter_get_column_type;
  iface->get_iter = gui_tree_model_filter_get_iter;
  iface->get_path = gui_tree_model_filter_get_path;
  iface->get_value = gui_tree_model_filter_get_value;
  iface->iter_next = gui_tree_model_filter_iter_next;
  iface->iter_children = gui_tree_model_filter_iter_children;
  iface->iter_has_child = gui_tree_model_filter_iter_has_child;
  iface->iter_n_children = gui_tree_model_filter_iter_n_children;
  iface->iter_nth_child = gui_tree_model_filter_iter_nth_child;
  iface->iter_parent = gui_tree_model_filter_iter_parent;
  iface->ref_node = gui_tree_model_filter_ref_node;
  iface->unref_node = gui_tree_model_filter_unref_node;
}

static void
gui_tree_model_filter_drag_source_init (GtkTreeDragSourceIface *iface)
{
  iface->row_draggable = gui_tree_model_filter_row_draggable;
  iface->drag_data_delete = gui_tree_model_filter_drag_data_delete;
  iface->drag_data_get = gui_tree_model_filter_drag_data_get;
}


static void
gui_tree_model_filter_finalize (GObject *object)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *) object;

  gui_tree_model_filter_set_model (filter, NULL);

  if (filter->priv->virtual_root)
    gtk_tree_path_free (filter->priv->virtual_root);

  if (filter->priv->root)
    gui_tree_model_filter_free_level (filter, filter->priv->root);
...
static void
gui_tree_model_filter_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  GuiTreeModelFilter *filter = GUI_TREE_MODEL_FILTER (object);

  switch (prop_id)
    {
      case PROP_CHILD_MODEL:
        gui_tree_model_filter_set_model (filter, g_value_get_object (value));
        break;
      case PROP_VIRTUAL_ROOT:
        gui_tree_model_filter_set_root (filter, g_value_get_boxed (value));
        break;
      default:
...
static void
gui_tree_model_filter_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
...
static void
gui_tree_model_filter_build_level (GuiTreeModelFilter *filter,
                                   FilterLevel        *parent_level,
                                   FilterElt          *parent_elt)
{
  GtkTreeIter iter;
  ...
  if (!parent_level)
  ...
  else
    {
      ...
      gui_tree_model_filter_convert_iter_to_child_iter (filter,
                                                        &child_parent_iter,
                                                        &parent_iter);
  ...
  do
    {
      if (gui_tree_model_filter_visible (filter, &iter))
        {
          ...
          if (GUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS (filter))
...
static void
gui_tree_model_filter_free_level (GuiTreeModelFilter *filter,
                                  FilterLevel        *filter_level)
{
  gint i;
  ...
  for (i = 0; i < filter_level->array->len; i++)
    {
      if (g_array_index (filter_level->array, FilterElt, i).children)
        gui_tree_model_filter_free_level (filter,
...
static GtkTreePath *
gui_tree_model_filter_elt_get_path (FilterLevel *level,
                                    FilterElt   *elt,
                                    GtkTreePath *root)
{
  ...
  if (root)
    {
      real_path = gui_tree_model_filter_add_root (path, root);
...
static GtkTreePath *
gui_tree_model_filter_add_root (GtkTreePath *src,
                                GtkTreePath *root)
{
  GtkTreePath *retval;
  gint i;
...
static GtkTreePath *
gui_tree_model_filter_remove_root (GtkTreePath *src,
                                   GtkTreePath *root)
{
  GtkTreePath *retval;
  gint i;
...
static void
gui_tree_model_filter_increment_stamp (GuiTreeModelFilter *filter)
{
  do
    {
      filter->priv->stamp++;
  ...
  gui_tree_model_filter_clear_cache (filter);
}

static gboolean
gui_tree_model_filter_visible (GuiTreeModelFilter *filter,
                               GtkTreeIter        *child_iter)
{
...
static void
gui_tree_model_filter_clear_cache_helper (GuiTreeModelFilter *filter,
                                          FilterLevel        *level)
{
  gint i;
  ...
  for (i = 0; i < level->array->len; i++)
    {
      if (g_array_index (level->array, FilterElt, i).zero_ref_count > 0)
        gui_tree_model_filter_clear_cache_helper (filter, g_array_index (level->array, FilterElt, i).children);
     }

  if (level->ref_count == 0 && level != filter->priv->root)
    {
      gui_tree_model_filter_free_level (filter, level);
      return;
    }
}

static FilterElt *
gui_tree_model_filter_fetch_child (GuiTreeModelFilter *filter,
                                   FilterLevel        *level,
                                   gint                offset,
                                   gint               *index)
{
  ...
  if (level->parent_elt)
    {
      c_parent_path =
        gui_tree_model_filter_elt_get_path (level->parent_level,
                                            level->parent_elt,
  ...
  if (offset >= len || !gui_tree_model_filter_visible (filter, &c_iter))
    return NULL;
...
static void
gui_tree_model_filter_remove_node (GuiTreeModelFilter *filter,
                                   GtkTreeIter        *iter,
                                   gboolean            emit_signal)
{
  ...
  while (elt->ref_count > 0)
    gui_tree_model_filter_real_unref_node (GTK_TREE_MODEL (filter),
                                           iter, FALSE);
  ...
  if (emit_signal)
    {
      ...
      gui_tree_model_filter_increment_stamp (filter);
...
static void
gui_tree_model_filter_update_childs (GuiTreeModelFilter *filter,
                                     FilterLevel        *level,
                                     FilterElt          *elt)
{
  GtkTreeIter c_iter;
  ...
  gui_tree_model_filter_convert_iter_to_child_iter (filter, &c_iter, &iter);
...
/* TreeModel signals */
static void
gui_tree_model_filter_row_changed (GtkTreeModel *c_model,
                                   GtkTreePath  *c_path,
                                   GtkTreeIter  *c_iter,
                                   gpointer      data)
{
  GuiTreeModelFilter *filter = GUI_TREE_MODEL_FILTER (data);
  GtkTreeIter iter;
  GtkTreeIter childs;
  ...
  requested_state = gui_tree_model_filter_visible (filter, &real_c_iter);
  ...
  if (current_state == TRUE && requested_state == FALSE)
    {
      ...
      gtk_tree_model_get_iter (GTK_TREE_MODEL (filter), &iter, path);
      gui_tree_model_filter_remove_node (filter, &iter, TRUE);
  ...
  if (current_state == TRUE && requested_state == TRUE)
    {
      ...
      if (gtk_tree_model_iter_children (c_model, &childs, &real_c_iter))
        gui_tree_model_filter_update_childs (filter, level, elt);
  ...
  if (!filter->priv->root)
    {
      ...
      gui_tree_model_filter_build_level (filter, NULL, NULL);
...
static void
gui_tree_model_filter_row_inserted (GtkTreeModel *c_model,
                                    GtkTreePath  *c_path,
                                    GtkTreeIter  *c_iter,
                                    gpointer      data)
{
  ...
  if (filter->priv->virtual_root)
    {
      real_path = gui_tree_model_filter_remove_root (c_path,
                                                     filter->priv->virtual_root);
      /* not our kiddo */
      if (!real_path)
        goto done;
  ...
  if (gui_tree_model_filter_visible (filter, &real_c_iter))
    {
      FilterElt felt;
...
static void
gui_tree_model_filter_row_has_child_toggled (GtkTreeModel *c_model,
                                             GtkTreePath  *c_path,
                                             GtkTreeIter  *c_iter,
                                             gpointer      data)
{
  ...
  if (!gui_tree_model_filter_visible (filter, c_iter))
    return;
...
static void
gui_tree_model_filter_row_deleted (GtkTreeModel *c_model,
                                   GtkTreePath  *c_path,
                                   gpointer      data)
{
  GuiTreeModelFilter *filter = GUI_TREE_MODEL_FILTER (data);
  ...
  if (emit_signal)
    {
      ...
      while (elt->ref_count > 0)
        gui_tree_model_filter_real_unref_node (GTK_TREE_MODEL (data), &iter,
                                               FALSE);
    }
...
static void
gui_tree_model_filter_rows_reordered (GtkTreeModel *c_model,
                                      GtkTreePath  *c_path,
                                      GtkTreeIter  *c_iter,
                                      gint         *new_order,
                                      gpointer      data)
  ...
  else
    {
      ...
      if (!path)
      ...
      else
        {
          ...
          gui_tree_model_filter_convert_iter_to_child_iter (GUI_TREE_MODEL_FILTER (filter), &child_iter, &iter);
...
static guint
gui_tree_model_filter_get_flags (GtkTreeModel *model)
{
  GtkTreeModelFlags flags;

  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), 0);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, 0);

  flags = gtk_tree_model_get_flags (GUI_TREE_MODEL_FILTER (model)->priv->child_model);
...
static gint
gui_tree_model_filter_get_n_columns (GtkTreeModel *model)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
...
static GType
gui_tree_model_filter_get_column_type (GtkTreeModel *model,
                                       gint          index)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;

  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), G_TYPE_INVALID);
...
static gboolean
gui_tree_model_filter_get_iter (GtkTreeModel *model,
                                GtkTreeIter  *iter,
                                GtkTreePath  *path)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
  ...
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), FALSE);
  ...
  for (i = 0; i < depth - 1; i++)
    {
      ...
      if (!g_array_index (level->array, FilterElt, indices[i]).children)
        gui_tree_model_filter_build_level (filter, level,
...
static GtkTreePath *
gui_tree_model_filter_get_path (GtkTreeModel *model,
                                GtkTreeIter  *iter)
{
  GtkTreePath *retval;
  FilterLevel *level;
  ...
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), NULL);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, NULL);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp, NULL);
...
static void
gui_tree_model_filter_get_value (GtkTreeModel *model,
                                 GtkTreeIter  *iter,
                                 gint          column,
                                 GValue       *value)
{
  GtkTreeIter child_iter;
  GuiTreeModelFilter *filter = GUI_TREE_MODEL_FILTER (model);

  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (model));
  g_return_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL);
  g_return_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp);
  ...
  gui_tree_model_filter_convert_iter_to_child_iter (GUI_TREE_MODEL_FILTER (model), &child_iter, iter);
  gtk_tree_model_get_value (GUI_TREE_MODEL_FILTER (model)->priv->child_model,
                            &child_iter, column, value);
}

static gboolean
gui_tree_model_filter_iter_next (GtkTreeModel *model,
                                 GtkTreeIter  *iter)
{
  FilterLevel *level;
  FilterElt *elt;
  ...
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), FALSE);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, FALSE);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp, FALSE);
...
static gboolean
gui_tree_model_filter_iter_children (GtkTreeModel *model,
                                     GtkTreeIter  *iter,
                                     GtkTreeIter  *parent)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
  ...
  if (!parent)
  ...
  else
    {
      if (FILTER_ELT (parent->user_data2)->children == NULL)
        gui_tree_model_filter_build_level (filter,
...
static gboolean
gui_tree_model_filter_iter_has_child (GtkTreeModel *model,
                                      GtkTreeIter  *iter)
{
  GtkTreeIter child_iter;
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
  ...
  filter = GUI_TREE_MODEL_FILTER (model);
  ...
  if (!elt->children
    ...
    gui_tree_model_filter_build_level (filter, FILTER_LEVEL (iter->user_data),
                                       elt);
...
static gint
gui_tree_model_filter_iter_n_children (GtkTreeModel *model,
                                       GtkTreeIter  *iter)
{
  GtkTreeIter child_iter;
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
...
static gboolean
gui_tree_model_filter_iter_nth_child (GtkTreeModel *model,
                                      GtkTreeIter  *iter,
                                      GtkTreeIter  *parent,
                                      gint          n)
{
  ...
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), FALSE);
  if (parent)
    g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->stamp == parent->stamp, FALSE);

  /* use this instead of has_Child to force us to build the level, if needed */
  if (gui_tree_model_filter_iter_children (model, &children, parent) == FALSE)
    {
      iter->stamp = 0;
      return FALSE;
    }
  ...
  iter->stamp = GUI_TREE_MODEL_FILTER (model)->priv->stamp;
  iter->user_data = level;
...
static gboolean
gui_tree_model_filter_iter_parent (GtkTreeModel *model,
                                   GtkTreeIter  *iter,
                                   GtkTreeIter  *child)
{
  FilterLevel *level;
  ...
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (model), FALSE);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, FALSE);
  g_return_val_if_fail (GUI_TREE_MODEL_FILTER (model)->priv->stamp == child->stamp, FALSE);
...
static void
gui_tree_model_filter_ref_node (GtkTreeModel *model,
                                GtkTreeIter  *iter)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
  GtkTreeIter child_iter;
...
static void
gui_tree_model_filter_unref_node (GtkTreeModel *model,
                                  GtkTreeIter  *iter)
{
  gui_tree_model_filter_real_unref_node (model, iter, TRUE);
}

static void
gui_tree_model_filter_real_unref_node (GtkTreeModel *model,
                                       GtkTreeIter  *iter,
                                       gboolean      propagate_unref)
{
  GuiTreeModelFilter *filter = (GuiTreeModelFilter *)model;
...
gui_tree_model_filter_row_draggable (GtkTreeDragSource *drag_source,
                                     GtkTreePath       *path)
{
  GuiTreeModelFilter *tree_model_filter = (GuiTreeModelFilter *)drag_source;
  GtkTreePath *child_path;
  gboolean draggable;

  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (drag_source), FALSE);
  g_return_val_if_fail (path != NULL, FALSE);

  child_path = gui_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
...
static gboolean
gui_tree_model_filter_drag_data_delete (GtkTreeDragSource *drag_source,
                                        GtkTreePath       *path)
{
  GuiTreeModelFilter *tree_model_filter = (GuiTreeModelFilter *)drag_source;
  GtkTreePath *child_path;
...
/* bits and pieces */
static void
gui_tree_model_filter_set_model (GuiTreeModelFilter *filter,
                                 GtkTreeModel       *child_model)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));
  ...
  if (child_model)
    {
      ...
      filter->priv->changed_id =
        g_signal_connect (child_model, "row_changed",
                          G_CALLBACK (gui_tree_model_filter_row_changed),
                          filter);
      filter->priv->inserted_id =
        g_signal_connect (child_model, "row_inserted",
                          G_CALLBACK (gui_tree_model_filter_row_inserted),
                          filter);
      filter->priv->has_child_toggled_id =
        g_signal_connect (child_model, "row_has_child_toggled",
                          G_CALLBACK (gui_tree_model_filter_row_has_child_toggled),
                          filter);
      filter->priv->deleted_id =
        g_signal_connect (child_model, "row_deleted",
                          G_CALLBACK (gui_tree_model_filter_row_deleted),
                          filter);
      filter->priv->reordered_id =
        g_signal_connect (child_model, "rows_reordered",
                          G_CALLBACK (gui_tree_model_filter_rows_reordered),
                          filter);
...
static void
gui_tree_model_filter_set_root (GuiTreeModelFilter *filter,
                                GtkTreePath        *root)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));
 ...
 */
GtkTreeModel *
gui_tree_model_filter_new (GtkTreeModel *child_model,
                           GtkTreePath  *root)
{
  GtkTreeModel *retval;
  ...
  retval = GTK_TREE_MODEL (g_object_new (gui_tree_model_filter_get_type (), NULL));

  gui_tree_model_filter_set_model (GUI_TREE_MODEL_FILTER (retval),
                                   child_model);
  gui_tree_model_filter_set_root (GUI_TREE_MODEL_FILTER (retval), root);

  return retval;
}

/**
 * gui_tree_model_filter_get_model:
 * @filter: A #GuiTreeModelFilter.
 *
...
{
  g_return_val_if_fail (GUI_IS_TREE_MODEL_FILTER (filter), NULL);

  return filter->priv->child_model;
}

/**
 * gui_tree_model_filter_set_visible_func:
 * @filter: A #GuiTreeModelFilter.
 * @func: A #GuiTreeModelFilterVisibleFunc, the visible function.
 ...
 */
void
gui_tree_model_filter_set_visible_func (GuiTreeModelFilter            *filter,
...
/**
 * gui_tree_model_filter_set_modify_func:
 * @filter: A #GuiTreeModelFilter.
 * @n_columns: The number of columns in the filter model.
 * @types: The #GType<!-- -->s of the columns.
 * @func: A #GuiTreeModelFilterModifyFunc, or %NULL.
 ...
 */
void
gui_tree_model_filter_set_modify_func (GuiTreeModelFilter           *filter,
                                       gint                          n_columns,
                                       GType                        *types,
...
/**
 * gui_tree_model_filter_set_visible_column:
 * @filter: A #GuiTreeModelFilter.
 ...
 */
void
gui_tree_model_filter_set_visible_column (GuiTreeModelFilter *filter,
                                          gint column)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));
 ...
 */
void
gui_tree_model_filter_convert_child_iter_to_iter (GuiTreeModelFilter *filter,
                                                  ...
                                                  GtkTreeIter        *child_iter)
{
  ...
  path = gui_tree_model_filter_convert_child_path_to_path (filter,
                                                           child_path);
  gtk_tree_path_free (child_path);
  g_return_if_fail (path != NULL);
...
/**
 * gui_tree_model_filter_convert_iter_to_child_iter:
 * @filter: A #GuiTreeModelFilter.
 ...
 */
void
gui_tree_model_filter_convert_iter_to_child_iter (GuiTreeModelFilter *filter,
                                                  GtkTreeIter        *child_iter,
                                                  GtkTreeIter        *filter_iter)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));
  ...
  if (GUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS (filter))
  ...
  else
    {
      ...
      path = gui_tree_model_filter_elt_get_path (filter_iter->user_data,
...
static GtkTreePath *
gtk_real_tree_model_filter_convert_child_path_to_path (GuiTreeModelFilter *filter,
                                                       GtkTreePath        *child_path,
                                                       gboolean            build_levels,
                                                       gboolean            fetch_childs)
{
  ...
  if (!filter->priv->virtual_root)
    ...
    real_path = gui_tree_model_filter_remove_root (child_path,
                                                   filter->priv->virtual_root);
  ...
  for (i = 0; i < gtk_tree_path_get_depth (real_path); i++)
    {
      ...
      if (tmp)
        {
          ...
          if (!tmp->children && build_levels)
            gui_tree_model_filter_build_level (filter, level, tmp);
          level = tmp->children;
          found_child = TRUE;
        }
      ...
      if (!found_child && fetch_childs)
        {
          tmp = gui_tree_model_filter_fetch_child (filter, level,
                                                   child_indices[i],
                                                   &j);
...
/**
 * gui_tree_model_filter_convert_child_path_to_path:
 * @filter: A #GuiTreeModelFilter.
 ...
 */
GtkTreePath *
gui_tree_model_filter_convert_child_path_to_path (GuiTreeModelFilter *filter,
                                                  GtkTreePath        *child_path)
{
 ...
 */
GtkTreePath *
gui_tree_model_filter_convert_path_to_child_path (GuiTreeModelFilter *filter,
  ...
  for (i = 0; i < gtk_tree_path_get_depth (filter_path); i++)
    {
      ...
      if (g_array_index (level->array, FilterElt, count).children == NULL)
        gui_tree_model_filter_build_level (filter, level, &g_array_index (level->array, FilterElt, count));
  ...
  if (filter->priv->virtual_root)
    {
      ...
      real_retval = gui_tree_model_filter_add_root (retval,
                                                    filter->priv->virtual_root);
      gtk_tree_path_free (retval);
...
static gboolean
gui_tree_model_filter_refilter_helper (GtkTreeModel *model,
                                       GtkTreePath  *path,
                                       GtkTreeIter  *iter,
                                       gpointer      data)
{
  /* evil, don't try this at home, but certainly speeds things up */
  gui_tree_model_filter_row_changed (model, path, iter, data);

  return FALSE;
}

/**
 * gui_tree_model_filter_refilter:
 * @filter: A #GuiTreeModelFilter.
 *
 ...
 */
void
gui_tree_model_filter_refilter (GuiTreeModelFilter *filter)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));

  /* S L O W */
  gtk_tree_model_foreach (filter->priv->child_model,
                          gui_tree_model_filter_refilter_helper,
                          filter);
}

/**
 * gui_tree_model_filter_clear_cache:
 * @filter: A #GuiTreeModelFilter.
 *
 ...
 */
void
gui_tree_model_filter_clear_cache (GuiTreeModelFilter *filter)
{
  g_return_if_fail (GUI_IS_TREE_MODEL_FILTER (filter));

  if (filter->priv->zero_ref_count)
    gui_tree_model_filter_clear_cache_helper (filter,
